...
from ...service.data_service import get_dataset_info
...
def get_one_data_file(request: Request, path: str, filename: str):
    lines = []
    data_path = my_settings.data_dir + path + "/" + filename
    print("show data_path: ", data_path)
...
async def upload_data_file(request: Request):
    ...
    target_folder, directory_name = create_directory(filename, my_settings.base_data_path)
    upload_file = os.path.join(target_folder, filename)

    if os.path.splitext(filename)[1] == ".xlsx":
        with open(my_settings.base_data_path + "/temp_file.xlsx", "wb") as temp_file:
            temp_file.write(file.read())
        contents = excel_to_json(my_settings.base_data_path + "/temp_file.xlsx")
